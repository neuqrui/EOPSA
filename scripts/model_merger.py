...
if __name__ == "__main__":
    ...
    # Backup generation_config.json before save_pretrained overwrites it
    # save_pretrained auto-generates a generation_config from model config,
    # which may lose custom eos_token_id settings (e.g. [151645, 151645, 151643] -> 151645)
    ...
    if os.path.exists(gen_config_path):
        ...
        with open(gen_config_path, "r") as f:
            gen_config_backup = json.load(f)

    print(f"Saving model to {hf_path}...")
    model.save_pretrained(hf_path, state_dict=state_dict)
    del state_dict, model

    # Restore original generation_config.json
    if gen_config_backup is not None:
        import json

        with open(gen_config_path, "w") as f:
            json.dump(gen_config_backup, f, indent=2)
        print(f"Restored original generation_config.json (eos_token_id={gen_config_backup.get('eos_token_id')})")
