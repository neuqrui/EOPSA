...
class FSDPCheckpointManager(BaseCheckpointManager):
    ...
    - fsdp_config.json (world_size / FSDP version) for model_merger
    """
    ...
    def load_checkpoint(self, path: Optional[str] = None):
        ...
        # save_model_only checkpoints have no optim/extra; resume with fresh optimizer.
        if not os.path.exists(optim_path) or not os.path.exists(extra_path):
            print(
                f"[rank-{self.rank}]: Optimizer/extra_state missing under {path}; "
                "loading model weights only (fresh optimizer / lr_scheduler)."
            )
            set_model_state_dict(
                model=self.model,
        ...
        if self.rank == 0:
            ...
            try:
                ...
                if isinstance(wrapped, PeftModel):
                    wrapped.save_pretrained(hf_path)
                elif isinstance(wrapped, PreTrainedModel):
                    wrapped.config.save_pretrained(hf_path)
                    wrapped.generation_config.save_pretrained(hf_path)
                else:
                    raise TypeError(f"Unsupported wrapped module type: {type(wrapped)}")
            except ImportError:
                assert isinstance(wrapped, PreTrainedModel)
                wrapped.config.save_pretrained(hf_path)
                wrapped.generation_config.save_pretrained(hf_path)
            self.processing_class.save_pretrained(hf_path)

            # Required by verl.model_merger (eval_llm_safety/run.sh merge step).
            fsdp_config_path = os.path.join(path, "fsdp_config.json")
            with open(fsdp_config_path, "w", encoding="utf-8") as f:
                json.dump({"FSDP_version": 1, "world_size": int(self.world_size)}, f, indent=4)
            print(f"[rank-0]: Saved FSDP config to {os.path.abspath(fsdp_config_path)}.")
