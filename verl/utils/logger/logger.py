...
class SwanlabLogger(Logger):
    def __init__(self, config: dict[str, Any]) -> None:
        if not is_package_available("swanlab"):
            raise ImportError(
                "swanlab is not installed. Install it or set trainer.logger without 'swanlab' "
                "(e.g. LOGGER=file)."
            )
        import swanlab  # type: ignore
    ...
    def __init__(self, loggers: Union[str, list[str]] = "console", config: Optional[dict[str, Any]] = None):
        ...
        for logger in loggers:
            ...
            try:
                ...
                # Optional backends (swanlab/wandb/...) may be missing on some machines.
                print(f"[WARN] skip logger '{logger}': {e}")

        if not self.loggers:
            print("[WARN] no loggers available; falling back to console.")
            self.loggers.append(ConsoleLogger(config or {}))
