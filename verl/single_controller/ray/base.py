...
def create_colocated_worker_cls(class_dict: dict[str, RayClassWithInitArgs]):
    ...
    cls in cls_dict
    """
    from ..base import Worker as BaseWorker

    cls_dict = {}
    init_args_dict = {}
    worker_cls = None
    for key, cls in class_dict.items():
        actor_cls = cls.cls.__ray_actor_class__
        # WorkerDict must subclass the base Worker (empty ctor). Using the immediate
        # parent breaks subclasses of FSDPWorker (parent would be FSDPWorker).
        if BaseWorker not in actor_cls.mro():
            raise TypeError(f"{actor_cls} must inherit from Worker, got MRO={actor_cls.mro()}")
