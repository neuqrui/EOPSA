...
# String metadata returned by some reward functions (e.g. safety RLSD verifier hints).
TEXT_REWARD_METRIC_KEYS = frozenset({"verifier_feedback", "think_diagnosis", "answer_diagnosis"})
...
def _is_numeric_metric_value(value: Any) -> bool:
    ...
    if isinstance(value, torch.Tensor):
        return value.numel() == 1 and not value.dtype.is_complex
    return False


def filter_numeric_reward_metrics(metrics: dict[str, list[Any]]) -> dict[str, list[Any]]:
    """Drop text/metadata reward keys before mean aggregation."""
    filtered: dict[str, list[Any]] = {}
    for key, values in metrics.items():
        if key in TEXT_REWARD_METRIC_KEYS:
            continue
        if values is None or len(values) == 0:
            continue
        first = values[0]
        if isinstance(first, (str, bytes, dict, list, tuple)):
            continue
        if isinstance(first, np.ndarray) and (first.dtype.kind in ("U", "S", "O") or first.size != 1):
            continue
        if not _is_numeric_metric_value(first):
            continue
        filtered[key] = values
    return filtered
