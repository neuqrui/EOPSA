...
class AdvantageEstimator(str, Enum):
    ...
    GAE = "gae"
    GRPO = "grpo"
    GRPO_SPLIT_SAFETY = "grpo_split_safety"
...
def _grpo_normalize_outcome_scores(scores: torch.Tensor, index, eps: float) -> torch.Tensor:
    """Group-wise GRPO normalization for 1-D outcome scores."""
    id2score = defaultdict(list)
    bsz = scores.shape[0]
    for i in range(bsz):
        id2score[index[i]].append(scores[i])
    ...
    for i in range(bsz):
        group_key = index[i]
        group_scores = torch.stack(id2score[group_key])
        assert group_scores.numel() > 1, "GRPO needs rollout.n > 1."
        mean = group_scores.mean()
        std = group_scores.std()
        normalized[i] = (scores[i] - mean) / (std + eps)
    return normalized


@register_adv_estimator(AdvantageEstimator.GRPO_SPLIT_SAFETY)
def compute_grpo_split_safety_advantage(
    think_scores: torch.Tensor,
    ...
    **kwargs,
    ...
    """Split think/answer outcome scores, GRPO-normalize separately, stitch token advantages.

    All masks and the returned ``advantages`` are aligned with ``batch["responses"]`` only
    (shape ``(bs, response_length)``). Prompt tokens never receive advantages.

    When ``unified_scale=True``, a second group-wise normalization is applied on the
    stitched advantage tokens to ensure think/answer/gap segments share the same scale.
    """
    if think_scores.dim() != 1:
        think_scores = think_scores.reshape(-1)
    if answer_scores.dim() != 1:
        answer_scores = answer_scores.reshape(-1)

    adv_think = _grpo_normalize_outcome_scores(think_scores, index, eps)
    adv_answer = _grpo_normalize_outcome_scores(answer_scores, index, eps)

    gap_mask = response_mask * (1.0 - think_mask) * (1.0 - answer_mask)
    gap_scores = (think_scores + answer_scores) * 0.5
    adv_gap = _grpo_normalize_outcome_scores(gap_scores, index, eps)

    advantages = (
        adv_think.unsqueeze(-1) * think_mask
        + adv_answer.unsqueeze(-1) * answer_mask
        + adv_gap.unsqueeze(-1) * gap_mask
    )
    advantages = advantages * response_mask

    if unified_scale:
        bsz = advantages.shape[0]
        # Collect all non-zero token advantages for each group
        id2token_advs: dict = defaultdict(list)
        for i in range(bsz):
            gid = index[i]
            token_advs = advantages[i][response_mask[i].bool()]
            if token_advs.numel() > 0:
                id2token_advs[gid].append(token_advs)
        id2mean: dict = {}
        id2std: dict = {}
        for gid in id2token_advs:
            all_advs = torch.cat(id2token_advs[gid])
            id2mean[gid] = all_advs.mean()
            id2std[gid] = all_advs.std()
        for i in range(bsz):
            gid = index[i]
            advantages[i] = ((advantages[i] - id2mean[gid]) / (id2std[gid] + eps)) * response_mask[i]
