...
def compute_advantage(
    ...
    if enable_split_safety_advantage:
        required = ("think_span_mask", "answer_span_mask", "think_outcome_scores", "answer_outcome_scores")
        missing = [key for key in required if key not in data.batch]
        if missing:
            raise ValueError(
                "enable_split_safety_advantage=True but batch is missing: "
                + ", ".join(missing)
            )
        advantages, returns = compute_advantage_return(
            AdvantageEstimator.GRPO_SPLIT_SAFETY,
            think_scores=data.batch["think_outcome_scores"],
            answer_scores=data.batch["answer_outcome_scores"],
            think_mask=data.batch["think_span_mask"],
            answer_mask=data.batch["answer_span_mask"],
            response_mask=data.batch["response_mask"],
            index=data.non_tensor_batch["uid"],
            eps=split_safety_adv_eps,
            unified_scale=split_safety_adv_unified_scale,
        )
        data.batch["advantages"] = advantages
        data.batch["returns"] = returns

        # Compute monitoring metrics for split safety advantage
        _compute_split_adv_monitoring_metrics(data)
        return data
...
def _compute_split_adv_monitoring_metrics(data: DataProto) -> None:
    """Compute per-segment advantage statistics for monitoring.

    Computes adv/think_adv_mean, adv/answer_adv_mean, adv/think_adv_std,
    adv/answer_adv_std from the computed advantages and stores them in
    ...
    think_token_advs = advantages[think_mask.bool()]
    answer_token_advs = advantages[answer_mask.bool()]
    gap_mask = response_mask * (1.0 - think_mask) * (1.0 - answer_mask)
    gap_token_advs = advantages[gap_mask.bool()]

    metrics: dict[str, float] = {}
    for name, advs in [("think", think_token_advs), ("answer", answer_token_advs), ("gap", gap_token_advs)]:
        if advs.numel() > 0:
            metrics[f"adv/{name}_adv_mean"] = advs.mean().item()
            metrics[f"adv/{name}_adv_std"] = advs.std().item()
        else:
            metrics[f"adv/{name}_adv_mean"] = 0.0
            metrics[f"adv/{name}_adv_std"] = 0.0

    # Also compute a scale ratio: think_mean / answer_mean (only if both non-zero)
    if metrics.get("adv/think_adv_mean", 0.0) != 0.0 and metrics.get("adv/answer_adv_mean", 0.0) != 0.0:
        metrics["adv/think_answer_mean_ratio"] = metrics["adv/think_adv_mean"] / abs(metrics["adv/answer_adv_mean"])
...
class RayPPOTrainer:
    ...
    def _load_training_token_stats(self) -> None:
        ...
        try:
            ...
            print(f"[PPO] Failed to load training_token_stats from {path}: {exc}")
            return
        for key in (
            "total_train_tokens",
        ...
        print(
            f"[PPO] Loaded training_token_stats: "
    ...
    def _update_training_token_stats(
        ...
        rollout_tokens = metrics.get("perf/rollout_response_tokens")
        train_tokens = metrics.get("perf/train_response_tokens")
        if rollout_tokens is None and batch is not None and "response_mask" in batch.batch:
            rollout_tokens = float(batch.batch["response_mask"].sum().item())
        if train_tokens is None:
            train_tokens = rollout_tokens

        data_count = metrics.get("perf/step_completion_count")
    ...
    def _validate(self) -> dict[str, Any]:
        ...
        val_reward_metrics = {
            ...
            for key, value in reduce_metrics(filter_numeric_reward_metrics(reward_metrics_lst)).items()
        }
    ...
    def _make_batch_data(self, metrics: dict[str, Any], reward_futures: list = None) -> DataProto:
        ...
        while True:
            ...
            if current_batch_size < rollout_batch_size:
                ...
                if self.config.algorithm.online_filtering:
                    metrics.update({f"reward/{k}": v for k, v in reduce_metrics(filter_numeric_reward_metrics(all_metrics)).items()})
                    # 记录细粒度计时到 metrics
    ...
    def _attach_split_safety_advantage_inputs(
        ...
        if "think_safe" not in reward_metrics or "ans_safe" not in reward_metrics:
            raise ValueError(
                ...
                "Use FORMAT_MODE=api in safety_reward.py."
            )
    ...
    def fit(self):
        ...
        while self.global_step < self.training_steps:
                ...
                with timer("adv", timing_raw):
                    ...
                    if reward_futures and len(reward_futures) > 0:
                        ...
                        with timer("reward", timing_raw):
                            ...
                            reward_metrics = {
                                f"reward/{k}": v
                                for k, v in reduce_metrics(filter_numeric_reward_metrics(latest_reward_metrics)).items()
                            }
                        ...
                        latest_reward_metrics = reward_metrics_raw
                        reward_metrics = {
                            f"reward/{k}": v
                            for k, v in reduce_metrics(filter_numeric_reward_metrics(reward_metrics_raw)).items()
                        }
                        metrics.update(reward_metrics)
            ...
            if "response_mask" in batch.batch:
                metrics["perf/rollout_response_tokens"] = float(batch.batch["response_mask"].sum().item())
                metrics["perf/train_response_tokens"] = metrics["perf/rollout_response_tokens"]
            metrics["perf/step_completion_count"] = float(len(batch))
